...
export default function Activity(params: Props): JSX.Element {
    ...
    const [productFirst, setProductFirst] = useState<boolean>(false);
    var QUESTIONS_ANSWERED: number = 0;
    var QUESTIONS_CORRECT: number = 0;

    var SETS: [p1: number | ReactElement, p2: number | ReactElement, p3: number | ReactElement, productFirst: boolean, answer: number][] = [];
    ...
    const MAX_DIGITS = doublesOf.toString().length + 2;
    ...
    function generateSets() {
        ...
        bitsToAdd.forEach((double) => {
            if(equationFormat.includes('productLast')) {
                if(missingPosition.includes('multiplicand')) {
                    SETS.push([input, double, double * double, false, double]);
                }
                if(missingPosition.includes('multiplier')) {
                    SETS.push([double, input, double * double, false, double]);
                }
                if(missingPosition.includes('product')) {
                    SETS.push([double, double, input, false, double * double]);
                }
            }
            if(equationFormat.includes('productFirst')) {
                if(missingPosition.includes('multiplicand')) {
                    SETS.push([double * double, input, double, true, double]);
                }
                if(missingPosition.includes('multiplier')) {
                    SETS.push([double * double, double, input, true, double]);
                }
                if(missingPosition.includes('product')) {
                    SETS.push([input, double, double, true, double * double]);
                }
            }
        });
    }

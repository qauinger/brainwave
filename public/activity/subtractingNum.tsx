...
export default function Activity(params: Props): JSX.Element {
    ...
    const MAX_DIGITS = minuend.toString().length;
    ...
    function generateSets() {
        ...
        if(typeof subtrahend === 'number') {
            bitsToSubtract.push(subtrahend);
        } else {
            bitsToSubtract = subtrahend;
        }

        bitsToSubtract.forEach((bit) => {
            for(var i = minuend; i - bit >= 0; i--) {
                if(equationFormat.includes('differenceLast')) {
                    if(missingPosition.includes('minuend')) {
                        SETS.push([input, bit, i - bit, false, i]);
                    }
                    if(missingPosition.includes('subtrahend')) {
                        SETS.push([i, input, i - bit, false, bit]);
                    }
                    if(missingPosition.includes('difference')) {
                        SETS.push([i, bit, input, false, i - bit]);
                    }
                }
                if(equationFormat.includes('differenceFirst')) {
                    if(missingPosition.includes('minuend')) {
